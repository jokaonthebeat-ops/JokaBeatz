  ...
  return (
    <section className="py-20 bg-card relative overflow-hidden">
      ...
      <div className="hidden md:block absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-10 -left-10 w-56 h-56 bg-primary/5 rounded-full blur-3xl animate-orbit" />
        <div className="absolute -bottom-10 -right-10 w-72 h-72 bg-primary/4 rounded-full blur-3xl animate-orbit-reverse" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {testimonials.map(({ quote, name, role, initials }) => (
            <div
              key={name}
              className="bg-secondary border-2 border-border rounded-2xl p-6 card-lift cursor-default hover:border-primary/30 transition-colors"
            >
              <div className="text-5xl font-black text-primary/20 leading-none mb-3">"</div>
              <p className="text-sm text-muted leading-relaxed italic mb-6">{quote}</p>
